...
class Convolution(Module):
    ...
    def forward(self,X,padding=True,lrp_aware=False):
        '''
        ...
        norm = X
        if padding:
            norm = np.pad(X, ((0,0), (hstride,hstride), (wstride,wstride), (0,0)), 'constant', constant_values=(0.,)) # FOR LENET MODELS
        ...
        if self.lrp_aware:
            ...
            for i in range(Hout):
                for j in range(Wout):
                    self.Y[:,i,j,:] = np.tensordot(norm[:, i*hstride:i*hstride+hf: , j*wstride:j*wstride+wf: , : ],self.W,axes = ([1,2,3],[0,1,2])) + self.B
    ...
    def _simple_lrp(self,R):
        ...
        for i in range(Hout-hf):
            for j in range(Wout-hf):
                if self.lrp_aware:
                    Z = self.Z[:,i,j,...]
                else:
                    
                    Z = self.W[na,...] * self.X[:, i*hstride:i*hstride+hf , i*hstride:i*hstride+hf  , : , na]
